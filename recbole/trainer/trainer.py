...
class Trainer(AbstractTrainer):
    ...
    def _calculate_time_deltas(self, interaction):
        """
        计算相邻行为间的时间间隔
        timestamps: 时间戳张量，可能是1D或2D
        item_seq: 物品序列，用于确定序列形状和填充位置
        返回: [batch_size, seq_len] 时间间隔矩阵
        """
        if isinstance(interaction, tuple):
            interaction = interaction[0]

        if 'timestamp_list' not in interaction:
            return None
        timestamps = interaction['timestamp_list']
        item_seq = interaction['item_id_list']

        # 2D时间戳序列 [batch_size, seq_len]
        batch_size, seq_len = timestamps.shape
        time_deltas = torch.zeros_like(timestamps, dtype=torch.float)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            # 新增：计算时间间隔并添加到interaction中
            if self.use_temporal:
                self._calculate_time_deltas(interaction)
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            # tools
            # check running_flag
            if(tool.checkRunningFlag(self.running_flag)):
                break
    ...
    def evaluate(
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            #  核心修改：在评估循环中处理时间间隔
            if self.use_temporal:
                # 处理全排序评估的数据格式 (interaction, history_index, positive_u, positive_i)
                interaction, history_index, positive_u, positive_i = batched_data
                self._calculate_time_deltas(interaction)
                batched_data = (interaction, history_index, positive_u, positive_i)
