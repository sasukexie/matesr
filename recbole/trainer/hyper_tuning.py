...
class HyperTuning(object):
    ...
    def __init__(
        ...
        if 'model' not in self.space:
            self.space['model'] = args.model
        if 'dataset' not in self.space:
            self.space['dataset'] = args.dataset
    ...
    def _print_result(result_dict: dict):
        ...
        print("current best valid result:", result_dict["best_valid_result"])
        print("current test result:", result_dict["test_result"])
        print("current test result:", "	".join([str(value) for metric, value in result_dict["test_result"].items()]))
        print()
    ...
    def export_result(self, output_file=None):
        ...
        with open(output_file, "w") as fp:
            for params in self.params2result:
                fp.write(f'result: {utils.get_local_time()}\n')
                fp.write('params: ' + params + "\n")
                fp.write(
                    "Valid result: "
                ...
                testRes = None
                for key in self.params2result[params]["test_result"]:
                    # tools
                    if (str.startswith(key, 'hit@')
                        ...
                        if testRes == None:
                            ...
                            testRes += '	'
                        testRes += str(self.params2result[params]["test_result"][key])

                fp.write(f"all result: {testRes + '	' + params}\n")
                fp.write('\n')
