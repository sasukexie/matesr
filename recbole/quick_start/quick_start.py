...
def run(model, dataset, config_file_list=None, config_dict=None, saved=True, nproc=1, world_size=-1, ip="localhost", port="1882", group_offset=0):
...
def run_recbole(model=None, dataset=None, config_file_list=None, config_dict=None, saved=True, queue=None):
    ...
    """
    # start common set
    tool.startCommonSet(model, dataset, config_dict)
    ...
    tool.joint_config(config, logger, set_color)
    ...
    # export data
    # tool.exportData(model, dataset, log_dir, config)
